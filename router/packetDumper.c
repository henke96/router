...
static void packetDumper_init(struct packetDumper *self, int32_t ifIndex) {
    ...
    CHECK(sys_ioctl(self->listenFd, SIOCGIFNAME, &self->ifreq), RES == 0);
    CHECK(sys_ioctl(self->listenFd, SIOCGIFHWADDR, &self->ifreq), RES == 0);
}
...
static void packetDumper_onListenFd(struct packetDumper *self, int32_t epollFd) {
    ...
    // Filter pcap traffic generated by us.
    // Based on `tcpdump -dd -s 0xdcba 'not ((ether src aa:bb:cc:dd:ee:ff and ip and (tcp src port 0xabcd or udp src port 123)) or (ether dst aa:bb:cc:dd:ee:ff and ip and (tcp dst port 0xabcd or udp dst port 123)))'`.
    struct sock_filter code[] = {
        { 0x20, 0, 0, 0x00000008 },
        { 0x15, 0, 2, mem_loadU32BE(&self->ifreq.ifr_addr[4]) },
        { 0x28, 0, 0, 0x00000006 },
        { 0x15, 4, 0, mem_loadU16BE(&self->ifreq.ifr_addr[2]) },
        { 0x20, 0, 0, 0x00000002 },
        { 0x15, 0, 22, mem_loadU32BE(&self->ifreq.ifr_addr[4]) },
        { 0x28, 0, 0, 0x00000000 },
        { 0x15, 0, 20, mem_loadU16BE(&self->ifreq.ifr_addr[2]) },
        { 0x28, 0, 0, 0x0000000c },
        ...
        { 0x15, 0, 1, 0x0000007b },
        { 0x6, 0, 0, 0x00000000 },
        { 0x6, 0, 0, sizeof(buffer) }
    };
    struct sock_fprog prog = {
        .len = hc_ARRAY_LEN(code),
        .filter = &code[0]
    };
    CHECK(sys_setsockopt(self->packetFd, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog)), RES == 0);
