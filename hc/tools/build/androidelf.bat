...
if defined LINK_LIBLOG (
    call "%root_dir%\tools\genLib\gen_so.bat" "%root_dir%\src\hc\linux\android\liblog.so.c" "%out_dir%\liblog.so"
    if not errorlevel 0 exit /b
    if errorlevel 1 exit /b
)
if defined LINK_LIBDL (
    call "%root_dir%\tools\genLib\gen_so.bat" "%root_dir%\src\hc\linux\android\libdl.so.c" "%out_dir%\libdl.so"
    if not errorlevel 0 exit /b
    if errorlevel 1 exit /b
)
if defined LINK_LIBANDROID (
    call "%root_dir%\tools\genLib\gen_so.bat" "%root_dir%\src\hc\linux\android\libandroid.so.c" "%out_dir%\libandroid.so"
    if not errorlevel 0 exit /b
    if errorlevel 1 exit /b
)
if defined LINK_LIBC (
    call "%root_dir%\tools\genLib\gen_so.bat" "%root_dir%\src\hc\linux\android\libc.so.c" "%out_dir%\libc.so"
    if not errorlevel 0 exit /b
    if errorlevel 1 exit /b
)
exit /b
