...
recipe_start
./configure --prefix="/path/that/doesnt/exist" --disable-nls --disable-dependency-tracking --without-libiconv-prefix --without-libintl-prefix
make -j "$NUM_CPUS"
mkdir -p "../$RECIPE_NAME/bin"
install -c ./src/xz/.libs/xz "../$RECIPE_NAME/bin/xz"
recipe_finish
